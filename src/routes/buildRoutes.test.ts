...
vi.mock("../modules/watchAnalysis/watchStatusRoute", () => ({watchStatusRoute: vi.fn(() => [{path: "/watch/status"}])}));
...
describe("buildRoutes", () => {
    ...
    it("registers all API routes with /v1 prefix in a single call", () => {
        ...
        // 3rd call is the versioned batch — 11 route groups × 1 route each
        const versionedRoutes = route.mock.calls[2][0];
        expect(versionedRoutes).toHaveLength(11);
        ...
        expect(versionedRoutes[10]).toEqual({path: "/v1/watch/status"});
    });
