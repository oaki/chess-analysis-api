...
export class HistoryController {
    ...
    async getAll(props: IGetAllProps) {
        ...
        return games.map((game) => ({...game, moves: JSON.parse(game.moves)}));
    }
    ...
    async get(props: IGetProps) {
        ...
        const game = await gameRepository.findOne({
            where: {user_id: props.userId, id: props.id}
        });
    ...
    async updateGame(props: IUpdateGameProps) {
        ...
        const game = await gameRepository.findOne(props.gameId, {relations: ["user"]});
